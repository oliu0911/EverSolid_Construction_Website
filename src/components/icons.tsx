import type { ReactNode } from 'react'

/**
 * Brand glyphs — the single source of truth for the site's UI icon set. Every
 * social / WhatsApp mark across the header, footer, and floating button draws
 * from here so the drawings never drift. Rendered in `currentColor`, sized via
 * the `size` prop, decorative (`aria-hidden`).
 */
interface GlyphProps {
  size?: number
  className?: string
}

function glyph(size: number, className: string | undefined, children: ReactNode) {
  return (
    <svg
      width={size}
      height={size}
      viewBox="0 0 24 24"
      fill="currentColor"
      aria-hidden="true"
      className={className}
    >
      {children}
    </svg>
  )
}

export function WhatsAppGlyph({ size = 16, className }: GlyphProps) {
  // Official WhatsApp brand symbol (simple-icons / WhatsApp vector) — the filled
  // chat bubble with the phone handset. Rendered white on brand-green, or in the
  // accent where it pairs with ink text.
  return glyph(
    size,
    className,
    <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z" />,
  )
}

export function FacebookGlyph({ size = 16, className }: GlyphProps) {
  return glyph(
    size,
    className,
    <path d="M22 12a10 10 0 1 0-11.6 9.9v-7H7.9V12h2.5V9.8c0-2.5 1.5-3.9 3.8-3.9 1.1 0 2.2.2 2.2.2v2.5h-1.3c-1.2 0-1.6.8-1.6 1.6V12h2.8l-.4 2.9h-2.4v7A10 10 0 0 0 22 12Z" />,
  )
}

export function InstagramGlyph({ size = 16, className }: GlyphProps) {
  return glyph(
    size,
    className,
    <path d="M12 2.2c2.7 0 3 0 4.1.1 1 0 1.5.2 1.9.4.5.2.8.4 1.2.8.4.4.6.7.8 1.2.2.4.3.9.4 1.9.1 1.1.1 1.4.1 4.1s0 3-.1 4.1c0 1-.2 1.5-.4 1.9a3.2 3.2 0 0 1-.8 1.2 3.2 3.2 0 0 1-1.2.8c-.4.2-.9.3-1.9.4-1.1.1-1.4.1-4.1.1s-3 0-4.1-.1c-1 0-1.5-.2-1.9-.4a3.2 3.2 0 0 1-1.2-.8 3.2 3.2 0 0 1-.8-1.2c-.2-.4-.3-.9-.4-1.9-.1-1.1-.1-1.4-.1-4.1s0-3 .1-4.1c0-1 .2-1.5.4-1.9.1-.5.4-.8.8-1.2.4-.4.7-.6 1.2-.8.4-.2.9-.3 1.9-.4C9 2.2 9.3 2.2 12 2.2Zm0 1.8c-2.7 0-3 0-4 .1-.9 0-1.3.2-1.6.4-.4.2-.6.3-.9.7-.3.3-.5.6-.7 1-.2.3-.3.7-.4 1.5-.1 1.1-.1 1.4-.1 4.1s0 3 .1 4.1c0 .9.2 1.3.4 1.6.2.4.3.6.7.9.3.3.6.5 1 .7.3.2.7.3 1.5.4 1.1.1 1.4.1 4.1.1s3 0 4.1-.1c.9 0 1.3-.2 1.6-.4.4-.2.6-.3.9-.7.3-.3.5-.6.7-1 .2-.3.3-.7.4-1.5.1-1.1.1-1.4.1-4.1s0-3-.1-4.1c0-.9-.2-1.3-.4-1.6a2.2 2.2 0 0 0-.7-.9 2.2 2.2 0 0 0-1-.7c-.3-.2-.7-.3-1.5-.4-1-.1-1.3-.1-4.1-.1Zm0 3.1a5 5 0 1 1 0 10 5 5 0 0 1 0-10Zm0 1.8a3.2 3.2 0 1 0 0 6.4 3.2 3.2 0 0 0 0-6.4Zm5.2-3a1.2 1.2 0 1 1 0 2.4 1.2 1.2 0 0 1 0-2.4Z" />,
  )
}